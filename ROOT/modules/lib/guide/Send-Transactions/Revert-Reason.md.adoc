...
In smart contracts, the https://solidity.readthedocs.io/en/v0.5.10/control-structures.html#revert[`revert`] operation triggers an exception to flag an error and revert the current call.
The EVM passes back to the client an optional string message containing information about the error.
...
Use the link:../../Reference/CLI/CLI-Syntax.md#revert-reason-enabled[`--revert-reason-enabled`] command line option to include the revert reason in the transaction receipt in Hyperledger Besu.
...
With revert reason enabled, the transaction receipt returned by link:../../Reference/API-Methods.md#eth_gettransactionreceipt[`eth_getTransactionReceipt`] includes the revert reason as an ABI-encoded string.
...
As described in the https://solidity.readthedocs.io/en/v0.5.10/control-structures.html#revert[Solidity documentation], the transaction receipt includes the revert reason as an ABI-encoded string consisting of:
...
Client libraries, such as web3j, do not support extracting the revert reason from the transaction receipt.
To extract the revert reason your Dapp must interact directly with Besu using a custom JSON \-> Object converter.
