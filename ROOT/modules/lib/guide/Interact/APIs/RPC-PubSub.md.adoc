...
Subscribe to events by using either RPC Pub/Sub over WebSockets or xref:../Filters/Accessing-Logs-Using-JSON-RPC.adoc[filters over HTTP].

Use RPC Pub/Sub over WebSockets to wait for events instead of polling for them.
For example, a Dapp can subscribe to logs and receive notification when a specific event occurs.
...
link:Using-JSON-RPC-API.md#http-and-websocket-requests[WebSockets] supports the RPC Pub/Sub API.

To create subscriptions, use `eth_subscribe`.
Once subscribed, the API publishes notifications using `eth_subscription`.

!!!note

 `eth_subscription` publishes notifications; you do not need to call `eth_subscription`.

Subscriptions couple with connections.
If a connection is closes, this removes all subscriptions created over the connection.

=== Subscription ID

`eth_subscribe` returns a subscription ID for each subscription created.
...
The new headers notification returns link:../../../Reference/API-Objects.md#block-object[block objects].
The second parameter is optional.
If specified, the notifications include whole link:../../../Reference/API-Objects.md#transaction-object[transaction objects], Otherwise, the notifications include transaction hashes.
...
To notify you about xref:../../../Concepts/Events-and-Logs.adoc[logs] included in new blocks, use the `logs` parameter with `eth_subscribe`.
You can specify a filter object to receive notifications only for logs matching your filter.
...
Returns only logs created from these addresses.
* `topics` - (optional) Returns only logs that match the link:../../../Concepts/Events-and-Logs.md#topic-filters[specified topics].

If a chain reorganization occurs, the subscription publishes notifications for logs from the old chain with the `removed` property in the link:../../../Reference/API-Objects.md#log-object[log object] set to `true`.
This means the subscription can publish notifications for multiple logs for the same transaction.

The logs subscription returns link:../../../Reference/API-Objects.md#log-object[log objects].
...
If a chain reorganization occurs, Besu resubmits transactions for inclusion in the new canonical chain.
...
Dropped transactions can be re-added to the transaction pool from a variety of sources.
For example, receiving a previously dropped transaction from a peer.
As a result, it's possible to receive multiple dropped transaction notifications for the same transaction.
...
When behind the chain head, the synchronizing subscription returns an object indicating the synchronization progress.
When fully synchronized, returns `false`.
...
To cancel a subscription, use the <<subscription-id,subscription ID>> with `eth_unsubscribe`.
Only the connection that created a subscription can unsubscribe from it.

`eth_unsubscribe` returns `true` if subscription succuessfully unsubscribed;
